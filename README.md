...
ADMIN_PASSWORD=guclu-bir-admin-sifre
ADMIN_SESSION_SECRET=uzun-rastgele-admin-session-secret
# Opsiyonel (yalniz debug simulate endpoint'i icin):
AI_ADMIN_TOKEN=guclu-bir-debug-token
```
...
- Auth: Admin session cookie + `x-csrf-token` (admin panel ile ayni guvenlik modeli)
- Env:
...
- `x-admin-token` bu endpoint'te artik kabul edilmez.
- Bu endpoint kritik oldugu icin sadece admin oturumu + CSRF ile calisir.
...
- Admin session cookie + `x-csrf-token`
- `x-admin-token` artik kabul edilmez.
...
- Upstash Redis (production/serverless icin zorunlu):
  - `UPSTASH_REDIS_REST_URL`
  - `UPSTASH_REDIS_REST_TOKEN`
  - `UPSTASH_REDIS_PREFIX` (opsiyonel, varsayilan: `drkynox`)
  - Production veya Vercel runtime'da bu degiskenler eksikse server startup'ta hard-fail olur.
  - Yalnizca development/test ortaminda in-memory fallback kullanilir.
...
- Gecici cache:
  ...
  - skor cache
  - Production/serverless'ta Redis zorunludur; process ici fallback sadece development/test icin kullanilir
...
- `POST /api/admin/workflow/daily/trigger`
  - Auth: Admin session cookie + `x-csrf-token`
...
Not: `x-admin-token` bu route'larda artik kabul edilmez.
