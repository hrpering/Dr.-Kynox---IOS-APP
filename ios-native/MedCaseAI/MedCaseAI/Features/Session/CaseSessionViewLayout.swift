...
extension CaseSessionView {
    var topCaseHeader: some View {
        HeroHeader(
            eyebrow: SpecialtyOption.label(for: config.specialty),
            title: activeHeadline,
            subtitle: (config.mode == .voice && !isTextFallbackMode)
                ? "Sesli vaka oturumu aktif. Hastayı dinleyip karar zincirini yönet."
                : "Yazılı vaka oturumu aktif. Mesaj akışıyla tetkik ve yönetim planını ilerlet.",
            icon: "cross.case.fill",
            metrics: [
                HeroMetricItem(title: "Mod", value: (config.mode == .voice && !isTextFallbackMode) ? "Sesli" : "Yazılı", icon: "waveform"),
                HeroMetricItem(title: "Mesaj", value: "\(vm.messages.count)", icon: "text.bubble"),
                HeroMetricItem(title: "Durum", value: hasStarted ? "Aktif" : "Hazırlık", icon: "bolt")
            ]
        ) {
            HStack {
                Spacer()
    ...
    var statusStrip: some View {
        StateCard(tone: sessionStatusTone) {
    ...
    private var sessionStatusTone: AppSemanticTone {
        if !hasStarted {
            return .warning
        }
